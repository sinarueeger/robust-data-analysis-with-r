...
`r icon::fa_twitter()` @sinarueeger

`r icon::fa_github()` @sinarueeger

`r icon::fa_link()` sinarueeger.github.io
...
`r icon::fa_twitter()` @imallieburns

`r icon::fa_github()` @allie-burns
...
# **3.** 
### [Workshop: Can we use R?](#part-3-workshop) 
...
<div class="figure">
<img src="https://github.com/allisonhorst/stats-illustrations/raw/master/openscapes/environmental-data-science-r4ds.png" width="500" alt="Cover image" />
...
<div class="figure">
<img src="https://raw.githubusercontent.com/allisonhorst/stats-illustrations/master/openscapes/starwars-rey-rstats.png" width="700" alt="Cover image" />
<p class="caption">Artwork by <a href="https://github.com/allisonhorst/stats-illustrations">@allison_horst</a> for <a href="https://www.openscapes.org/">openscapes</a></p>
</div>
...
| *Functions*  |  *Purpose* |
|---|---|
| 1. Contigency table of `var`  | A. `group_by(var)` |
| 2. Extract all rows that have values > 2 |  B. `summarise(mean(var))`|
| 3. Create a new variable from `var_0` and `var_1`| C. `select(var)` |
| 4. Rename a column | D. `filter(var > 2)` |
| 5. Extract row #3 | E. `group_by(var) %>% summarize(mean(x))` |
| 6. Group by a variable | F. `rename(new_name = old_name)` |
| 7. Calculate the mean of a variable | G. `slice(3)` |
| 8. Select a column | H. `tabyl(var)` |
| 9. Calculate the mean of `x` for each level in `var` | I. `mutate(var = var_0 + var_1)` |
...
| *Functions*  |  *Purpose* |
|---|---|
| 1. Contigency table of `var`  | H. `tabyl(var)` |
| 2. Extract all rows that have values > 2 |  D. `filter(var > 2)`|
| 3. Create a new variable from `var_0` and `var_1`| I. `mutate(var = var_0 + var_1)` |
| 4. Rename a column | F. `rename(new_name = old_name)` |
| 5. Extract row #3 | G. `slice(3)` |
| 6. Group by a variable | A. `group_by(var)` |
| 7. Calculate the mean of a variable | B. `summarise(mean(var))` |
| 8. Select a column | C. `select(var)` |
| 9. Calculate the mean of `x` for each level in `var` | E. `group_by(var) %>% summarize(mean(x))` |
