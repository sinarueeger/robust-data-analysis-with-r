...
In the last script, we will bundle everything we learned into one R script. You might notice that this has not the typical `.R` file extension, but instead `.Rmd`. `.Rmd` means RMarkdown and combines the power of [Markdown](https://en.wikipedia.org/wiki/Markdown) and R. This form of mixing programming and writing is also called _literal programming_. 
